 ...
 */
if ( !function_exists('tainacan_evently_archive_templates_redirects') ) {
    function tainacan_evently_archive_templates_redirects() {
        global $wp_query;
        ...
        if (is_post_type_archive()) {
            ...
            if (in_array($current_post_type, $collections_post_types)) {			
                include( TAINACAN_EVENTLY_PLUGIN_DIR_PATH . 'tainacan/archive-items.php' );
                exit; 
            } 
            // else if ($current_post_type === 'tainacan-collection') {
            //     include( TAINACAN_EVENTLY_PLUGIN_DIR_PATH . 'tainacan/archive-collections.php' );
            //     exit;
            // }
        } else if ( is_tax() ) {
            ...
            if ( isset($term->taxonomy) && \Tainacan\Theme_Helper::get_instance()->is_taxonomy_a_tainacan_tax($term->taxonomy)) {
                ...
                include( TAINACAN_EVENTLY_PLUGIN_DIR_PATH . 'tainacan/archive-taxonomy.php' );
                exit;
            }
        } else if ( $wp_query->get( 'tainacan_repository_archive' ) == 1 ) {
            
            include( TAINACAN_EVENTLY_PLUGIN_DIR_PATH . 'tainacan/archive-repository.php' );
            exit;
			...
			if ( in_array($post_type, $collections_post_types)  ) {
                include(TAINACAN_EVENTLY_PLUGIN_DIR_PATH . 'tainacan/item-single-page.php' );
                exit;
			}
        }
...
add_action('template_redirect', 'tainacan_evently_archive_templates_redirects');


/**
 * Adds extra class to help styling tainacan archive templates.
 */
if ( !function_exists('tainacan_evently_post_class') ) {
    function tainacan_evently_post_class($classes) {
...
add_filter('body_class', 'tainacan_evently_post_class');
